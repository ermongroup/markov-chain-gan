...
class Trainer(object):
    def __init__(self, transition_fn, discriminator,
        ...
        with tf.variable_scope(self.name):
            ...
            xx_ = tf.reshape(tf.concat([self.z2, self.z3], 1), [-1, self.input_size])
            xz_ = self.z1
            self.d, _ = discriminator(x, reuse=False)
            self.dz_, _ = discriminator(xz_)
            self.dx_, _ = discriminator(xx_)
            self.dz_ = self.dz_[:, 0]
            self.dx_ = self.dx_[:, 1]

            # Wasserstein GAN objectives.
            self.d_loss = tf.reduce_mean(self.d) * 2. - tf.reduce_mean(self.dz_) - tf.reduce_mean(self.dx_)
            self.g_loss = tf.reduce_mean(self.dz_) + tf.reduce_mean(self.dx_)

            # Gradient Penalty.
            epsilon = tf.random_uniform([], 0.0, 1.0)
            xz_hat = xlz * epsilon + xz_ * (1 - epsilon)
            dz_hat, _ = discriminator(xz_hat)
            dz_hat = dz_hat[:, 0]

            epsilon = tf.random_uniform([], 0.0, 1.0)
            xx_hat = xlx * epsilon + xx_ * (1 - epsilon)
            dx_hat, _ = discriminator(xx_hat)
            dx_hat = dx_hat[:, 1]

            ddz = tf.gradients(dz_hat, xz_hat)[0]
            ddz = tf.sqrt(tf.reduce_sum(tf.square(ddz), axis=1))
            ddz = tf.reduce_mean(tf.square(ddz - 1.0) * scale)

            ddx = tf.gradients(dx_hat, xx_hat)[0]
            ddx = tf.sqrt(tf.reduce_sum(tf.square(ddx), axis=1))
            ddx = tf.reduce_mean(tf.square(ddx - 1.0) * scale)
            self.d_loss = self.d_loss + ddx + ddz
            ...
            try:
                ...
                for t in range(0, num_batches):
                    ...
                    if t % epoch_size == 0 and t > 0:
                        visualizer(self, '{}/{}-{}-{}'.format(path, t/epoch_size, self.b_, self.m_))

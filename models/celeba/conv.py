...
class TransitionFunction(TransitionBase):
    ...
    def __call__(self, x):
        ...
        c1 = tcl.flatten(c1)
        z = dense(c1, 200, activation_fn=tcl.batch_norm) + 0.05 * tf.random_normal(tf.stack([bs, 200]), mean=0.0, stddev=1)
        c1 = dense(z, 16 * 16 * 64)
...
class Discriminator(DiscriminatorBase):
    def __init__(self, name='celeba/d'):
        DiscriminatorBase.__init__(self, 32 * 32 * 3, 1, name)
    ...
    def __call__(self, x, reuse=True):
        with tf.variable_scope(self.name) as vs:
            ...
            y = dense(conv3, 1, activation_fn=tf.identity)
...
def visualizer(model, name):
    ...
    #images_one_step(model, fig, 100, [1, 2, 5, 10, 20, 50], [32, 32, 3], data_to_image=data_to_image)
    #fig.savefig('{}_mc.pdf'.format(name))
